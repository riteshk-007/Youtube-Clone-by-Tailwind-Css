...
function SearchResultVideoCard({ video }) {
  return (
    <Link to={`/video/${video?.videoId}`}>
      <div className="flex flex-col md:flex-row mb-8 md:mb-3 dark:lg:hover:bg-white/[0.1]  lg:hover:bg-black/[0.1] rounded-xl md:p-4">
            ...
            <div className="flex flex-col">
              <span className="text-sm font-semibold mt-2 text-black/[0.7] dark:text-white/[0.7] flex items-center">
                ...
                {video?.author?.badges[0]?.type === "VERIFIED_CHANNEL" && (
                  <BsFillCheckCircleFill className="text-black/[0.7] dark:text-white/[0.5] text-[12px] lg:text-[10px] xl:text-[12px] ml-1" />
                )}
              </span>
